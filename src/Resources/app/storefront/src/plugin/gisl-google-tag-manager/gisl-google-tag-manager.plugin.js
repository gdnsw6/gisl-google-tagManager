...
{
    init() {
        this.cookieEnabledName = 'gislAllowGtmTracking';
    ...
    fireSelectItemEvent(event) {
        ...
        let product= {
            ...
            'item_id': productBox.querySelector('input[name="gisl-gtm-product-sku"]').value,
        };

        let variantname = productBox.querySelector('input[name="gisl-gtm-product-variantname"]');
        let category = productBox.querySelector('input[name="gisl-gtm-product-category"]');
        let price = productBox.querySelector('input[name="gisl-gtm-product-price"]');
        let brand = productBox.querySelector('input[name="gisl-gtm-product-brand"]');
    ...
    onWishlistRemoveFormSubmit(event) {
        ...
        let skuField = DomAccessHelper.querySelector(event.target, 'input[name="gisl-gtm-product-sku"]', false);
        this.fireWishlistEvent(skuField, 'remove_from_wishlist');
    ...
    getLineItems() {
        ...
        lineItemDataElements.forEach(itemEl => {
            lineItems.push({
                item_id: DomAccessHelper.getDataAttribute(itemEl, 'data-gisl-sku'),
                item_name: DomAccessHelper.getDataAttribute(itemEl, 'name'),
                quantity: DomAccessHelper.getDataAttribute(itemEl, 'quantity'),
                price: DomAccessHelper.getDataAttribute(itemEl, 'data-gisl-price'),
            });
        });
    ...
    getSkuFromEvent(event) {
        ...
        if(siblingHiddenField) {
            return DomAccessHelper.querySelector(siblingHiddenField.parentNode, 'input[name="gisl-gtm-product-sku"]', false);
        }
